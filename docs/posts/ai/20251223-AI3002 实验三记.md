...
2. 对于 $t = 1,\cdots, T$：
    ...
    - $\alpha_i \gets \begin{cases}\alpha_i e^{-\hat{w}t} & f_t(x_i)=y_i\\ \alpha_i e^{\hat{w}t} & f_t(x_i)\ne y_i\end{cases}$；
    - 归一化 $\alpha_i$；
...
&\le \prod_{t=1}^T\sum_{i=1}^n\alpha_{i,t}\exp\left(-y_i\hat{w}_tf_t(x_i)\right)\triangleq\prod_{t=1}^TZ_t
\end{aligned}
$$

我们考虑通过控制 $\prod_{t=1}^TZ_t$ 的方式控制训练误差，使用贪心的策略，每次优先选择最小的 $Z_t$。
