...
Since $\mathcal T^\ast$ is a [contraction](https://en.wikipedia.org/wiki/Contraction_(operator_theory)), the iteration process $V_{k+1}=\mathcal T^{\ast}V_k$ converges—**value iteration**, however, the expectations are not explicitly solvable.

Therefore we employ [Robbins-Monro stochastic approximation](https://en.wikipedia.org/wiki/Stochastic_approximation#Robbins–Monro_algorithm) to $Q^\ast = \mathcal T^\ast Q^\ast$, obtaining:
...
Policy-based methods are *softened* or *approximate* policy iteration. They are often used in scenarios where the action space is large / continuous.
...
The derivation of $J$ relies in the *state distribution* thus intractable, however, by the **Policy Gradient Theorem**[^4], since $Q^{\pi_\theta}$ is a Bellman fixed point:
...
$$
\max_{\pi}\mathbb E_{a\sim\pi}[Q(s, a) - \alpha \log \pi(a|s)] = \alpha\log \sum_{a}\exp{Q(s,a)\over a}
$$
