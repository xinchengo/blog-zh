...
Similarly, Robbins-Monro approximation can be employed on $V^\pi = \mathcal T^\pi V^\pi$, resulting in **Temporal Difference Learning, TD**:

$$
V(s_t) \gets V(s_t)+\alpha\left[r_t+\gamma V(s_{t+1})-V(s_t)\right]
$$

> The formations of TD learning and Q learning are analogous.
